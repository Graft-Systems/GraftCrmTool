...
export async function applyCaptureReviewAction(companyId: string, formData: FormData) {
  ...
  if (parsed.applyNeeds && needsBullets.length > 0) {
    ...
    await recordActivity({
      ...
      kind: "needs_updated",
      summary: "Updated company needs from AI review.",
    });
  }
  ...
  if (parsed.applyTags && tagHints.length > 0) {
    ...
    await recordActivity({
      ...
      actorId: session.user.id,
      kind: "tags_updated",
      summary: "Updated company tags from AI review.",
    });
  }
  ...
  for (const task of selectedTasks) {
    ...
    await recordActivity({
      ...
      actorId: session.user.id,
      kind: "task_created",
      summary: `Created follow-up task: ${createdTask.title}`,
      metadata: { taskId: createdTask.id, interactionId: interaction.id },
    });
  }
  ...
  await recordActivity({
    companyId,
    actorId: session.user.id,
    kind: "ai_note_applied",
    summary: `Saved ${parsed.source === "in_app_voice" ? "voice" : "pasted"} notes after AI review.`,
    metadata: { interactionId: interaction.id, type: parsed.type, source: parsed.source },
  });
